...
READ_CONTRACTS = {
    ...
    "accounting.delivery_notes.search": _contract("read", ("companyId",), ("seenStatus", "query", "limit", "cursor"), "seenStatus: seen|unseen"),
    ...
    "accounting.documents.create_access_urls": _contract(
        ...
        ("ttlSeconds",),
        "Returns a view URL and, when available, a distinct original-file URL. Both are reusable until their default/maximum 10800-second expiry. XML invoice views use AssoSoftware HTML, PDF views are inline, and documents without a primary attachment use printable HTML. Consume capability URLs without the agent token, cookies, custom authorization headers or redirects",
    ),
    "accounting.documents.get": _contract("read", ("companyId", "documentId")),
    "accounting.documents.search": _contract("read", ("companyId", "types"), ("seenStatus", "paymentLinkStatus", "query", "limit", "cursor"), "types: 1..6 values among invoice_in, invoice_out, invoice_non_electronic_in, invoice_notice_in, credit_note, other_expense; seenStatus: seen|unseen; paymentLinkStatus: linked|unlinked|partial"),
    ...
    "accounting.payments.search": _contract("read", ("companyId",), ("direction", "seenStatus", "linkStatus", "query", "limit", "cursor"), "direction: in|out; seenStatus: seen|unseen; linkStatus: linked|unlinked|partial"),
...
PAYMENT_FIELDS = ("accountingEntryId", "insurancePolicyId", "accountingContactId", "accountId", "direction", "paymentDate", "amount", "excludeFromAccountBalance", "isAccountingCost", "costCategory", "costCompetenceMode", "costCompetenceFiscalYear", "costCompetenceStartDate", "costCompetenceEndDate", "costCompetenceAllocations", "description", "referenceNumber", "notes")
...
DRAFT_CONTRACTS = {
    ...
    "accounting.documents.recalculate_self_invoice": _contract("draft_with_confirmation", ("companyId", "documentId"), (), "Mandatory user confirmation even for direct-granted agents. Recover fiscal type from an authorized existing XML when needed; recalculate only eligible self-invoices with settled linked origins. No payment created, no date/rate/IVA/paidAmount changes. Do not use full XML reimport or force-paid as fallback."),
    ...
    "accounting.payments.create": _contract("draft_with_confirmation", ("companyId", "direction", "paymentDate", "amount"), tuple(field for field in PAYMENT_FIELDS if field not in {"direction", "paymentDate", "amount"}), "direction: in|out; insurancePolicyId links a same-company policy after server-side authorization"),
    "accounting.payments.update": _contract("draft_with_confirmation", ("companyId", "paymentId"), PAYMENT_FIELDS, "insurancePolicyId links a same-company policy; null removes the existing policy link"),
