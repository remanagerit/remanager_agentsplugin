...
The connector upper bound is 89 tools: 36 reads, 50 generic `draft_with_confirmation` actions and three file actions. It exposes zero `direct` tools and cannot approve an action. Discovery, current user permissions, grants, capabilities, company resources, rate limits and audit remain authoritative on the REmanager server.
...
The static 89-tool catalog is only an upper bound and never grants access. A tool must also be returned by REmanager discovery with the expected mode. Inputs cannot contain user, team, agent, scope, grant or execution-mode context at any nesting level.
